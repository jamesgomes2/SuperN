...
fun_udp1 () {
    ...
    fun_udpon () {
        ...
        [[ $(grep -wc "udpvpn" /etc/autostart) = '0' ]] && {
		    echo -e "screen -dmS udpvpn /bin/badvpn-udpgw --listen-addr 127.0.0.1:7300 --max-clients 1000 --max-connections-for-client 10" >> /etc/autostart
		} || {
    ...
    menu
	    ...
	    inst_udp () {
	        cd $HOME
            wget https://www.dropbox.com/s/6yqibjc0hg1iswk/badvpn-udpgw -o /dev/null
